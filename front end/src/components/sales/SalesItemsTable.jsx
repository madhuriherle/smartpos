...
export default function SalesItemsTable({ items, onRemove }) {
  return (
        ...
        <tbody>
          {items.length === 0 && (
            <tr>
              <td colSpan={10} className="px-5 py-8 text-center text-sm text-slate-400">
                No products added yet. Use the form above to add sales items.
              </td>
            </tr>
          )}
          ...
          {items.map((item, index) => (
            <tr key={index} className="border-b border-slate-50 text-slate-700 last:border-0 hover:bg-slate-50/60">
              ...
              <td className="px-5 py-3.5 text-right tabular-nums">{formatCurrency3(item.price)}</td>
